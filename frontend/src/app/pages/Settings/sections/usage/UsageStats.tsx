    ...
    <Box>
      ...
      {!stats ? (
        ...
        <>
          ...
          {stats.cost_by_provider && Object.keys(stats.cost_by_provider).length > 0 && (
            <>
              <Typography sx={sectionSx}>By lane (which credential served the requests)</Typography>
              <BarSeries
                data={Object.entries(stats.cost_by_provider)
                  .sort(([, a], [, b]) => b.requests - a.requests)
                  .map(([provider, v]) => ({ label: laneLabel(provider), value: v.requests, suffix: 'requests' }))}
              />
            </>
          )}
        </>
