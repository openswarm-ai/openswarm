...
export const fetchSessions = createAsyncThunk(
  ...
  async ({ dashboardId }: { dashboardId?: string } = {}) => {
    ...
    const res = await fetch(`${AGENTS_API}/get_all_sessions${qs ? `?${qs}` : ''}`);
...
export const launchAgent = createAsyncThunk('agents/launchAgent', async (config: AgentConfig) => {
  const res = await fetch(`${AGENTS_API}/launch_agent`, {
    method: 'POST',
...
export const sendMessage = createAsyncThunk(
  ...
  async ({ sessionId, prompt, mode, model, provider, images, contextPaths, forcedTools, attachedSkills, hidden, selectedBrowserIds }: SendMessagePayload) => {
    await fetch(`${AGENTS_API}/send_message`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ session_id: sessionId, prompt, mode, model, provider, images, image_media_types: images?.map(i => i.media_type), context_paths: contextPaths, forced_tools: forcedTools, attached_skills: attachedSkills, hidden, selected_browser_ids: selectedBrowserIds }),
    });
...
export const stopAgent = createAsyncThunk(
  ...
  async ({ sessionId, removeWorktree = false }: { sessionId: string; removeWorktree?: boolean }) => {
    await fetch(`${AGENTS_API}/stop_agent`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ session_id: sessionId, remove_worktree: removeWorktree }),
    });
    return sessionId;
  }
);
...
export const editMessage = createAsyncThunk(
  ...
  async ({ sessionId, messageId, content }: { sessionId: string; messageId: string; content: string }) => {
    await fetch(`${AGENTS_API}/edit_message`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
...
export const switchBranch = createAsyncThunk(
  ...
  async ({ sessionId, branchId }: { sessionId: string; branchId: string }) => {
    await fetch(`${AGENTS_API}/switch_branch`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
...
export const launchAndSendFirstMessage = createAsyncThunk(
  ...
  async ({ draftId, config, prompt, mode, model, provider, images, contextPaths, forcedTools, attachedSkills, selectedBrowserIds }: LaunchAndSendPayload) => {
    const launchRes = await fetch(`${AGENTS_API}/launch_agent`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify(config),
    });
    ...
    await fetch(`${AGENTS_API}/send_message`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ session_id: session.id, prompt, mode, model, provider, images, image_media_types: images?.map(i => i.media_type), context_paths: contextPaths, forced_tools: forcedTools, attached_skills: attachedSkills, selected_browser_ids: selectedBrowserIds }),
    });
...
export const generateTitle = createAsyncThunk(
  ...
  async ({ sessionId, prompt }: { sessionId: string; prompt: string }) => {
    // Title generation not implemented in backend yet - return truncated prompt
...
export const generateGroupMeta = createAsyncThunk(
  ...
  async ({ sessionId, groupId, toolCalls, resultsSummary, isRefinement }: GenerateGroupMetaPayload) => {
    // Group meta generation not implemented in backend yet - return placeholder
    const name = toolCalls.length > 0 ? toolCalls[0].tool : 'Tools';
    return { sessionId, groupId, name, svg: '', isRefined: isRefinement ?? false };
  }
);
...
export const updateSystemPrompt = createAsyncThunk(
  ...
  async ({ sessionId, systemPrompt }: { sessionId: string; systemPrompt: string }) => {
    await fetch(`${AGENTS_API}/update_system_prompt`, {
      method: 'PATCH',
      headers: { 'Content-Type': 'application/json' },
...
export const updateThinkingLevel = createAsyncThunk(
  ...
  async ({ sessionId, level }: { sessionId: string; level: 'off' | 'low' | 'medium' | 'high' | 'auto' }) => {
    // Thinking level update not implemented in backend yet - just return the level
...
export const handleApproval = createAsyncThunk(
  ...
  async ({
    ...
    const res = await fetch(`${AGENTS_API}/handle_approval`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
...
export const closeSession = createAsyncThunk(
  ...
  async ({ sessionId }: { sessionId: string }) => {
    await fetch(`${AGENTS_API}/close_session`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
...
export const duplicateSession = createAsyncThunk(
  ...
  async ({ sessionId, dashboardId, upToMessageId }: { sessionId: string; dashboardId?: string; upToMessageId?: string }) => {
    const res = await fetch(`${AGENTS_API}/duplicate_session`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ session_id: sessionId, dashboard_id: dashboardId, up_to_message_id: upToMessageId }),
    });
...
export const deleteSession = createAsyncThunk(
  ...
  async ({ sessionId }: { sessionId: string }) => {
    await fetch(`${AGENTS_API}/delete_session`, {
      method: 'DELETE',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ session_id: sessionId }),
    });
...
export const fetchHistory = createAsyncThunk(
  ...
  async ({ dashboardId }: { dashboardId?: string } = {}) => {
    ...
    if (dashboardId) params.set('dashboard_id', dashboardId);
    const res = await fetch(`${AGENTS_API}/get_history?${params}`);
    const data = await res.json();
...
export const resumeSession = createAsyncThunk(
  ...
  async ({ sessionId }: { sessionId: string }) => {
    const res = await fetch(`${AGENTS_API}/resume_session`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ session_id: sessionId }),
    });
...
export const fetchBrowserAgentChildren = createAsyncThunk(
  ...
  async (parentSessionId: string) => {
    // Browser agents endpoint not implemented yet - return empty array
