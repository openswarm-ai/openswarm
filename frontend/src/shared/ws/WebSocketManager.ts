...
export const dashboardWs = new WebSocketManager(`${WS_BASE}/api/agents/ws`, { skipStreamEvents: true });

export function createSessionWs(sessionId: string): WebSocketManager {
  // Per-session WebSocket not implemented yet, use the main agents ws
  return new WebSocketManager(`${WS_BASE}/api/agents/ws`);
}
