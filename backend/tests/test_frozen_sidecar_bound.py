...
from backend.apps.agents.manager.streaming import unwedge_sidecar as uw
...
def test_a_blocking_tool_is_exempt_from_the_quick_deadline():
    ...
    assert uw.is_quick_core_tool("mcp__openswarm-core__CreateBrowserAgent") is False
...
def test_a_stale_heartbeat_reads_as_dead(monkeypatch):
    ...
    assert uw.p_sidecar_is_dead("s1") is True
...
def test_a_beating_sidecar_is_never_called_dead(monkeypatch):
    ...
    assert uw.p_sidecar_is_dead("s1") is False
...
def test_the_watchdog_consults_liveness_not_only_settledness():
    src = open("backend/apps/agents/manager/streaming/unwedge_sidecar.py").read()
    i_settled = src.index("settled = delegation_children_settled(session_id, started)")
    i_dead = src.index("p_sidecar_is_dead(session_id)")
...
def test_a_live_child_keeps_the_call_alive_however_quiet_the_sidecar_looks(monkeypatch):
    ...
    try:
        assert uw.p_no_child_ever_born("par-live", started) is False, \
...
def test_a_child_from_an_EARLIER_delegation_does_not_count(monkeypatch):
    ...
    try:
        assert uw.p_no_child_ever_born("par-old", time.time()) is True
    finally:
