...
def test_a_real_windows_blob_decrypts(on_windows):
    ...
    assert bc.decrypt_value(p_win_blob(b"sessionid=abc123"), KEY) == "sessionid=abc123"
...
def test_the_32_byte_domain_hash_prefix_is_stripped(on_windows):
    ...
    assert bc.decrypt_value(p_win_blob(b"tok=xyz", prefix=b"\xa5" * 32), KEY) == "tok=xyz"
...
def test_a_wrong_key_returns_none_instead_of_raising(on_windows):
    ...
    assert bc.decrypt_value(p_win_blob(b"secret"), bytes(32)) is None
...
def test_app_bound_v20_is_refused_on_windows_too(on_windows):
    ...
    assert bc.decrypt_value(b"v20" + NONCE + b"whatever", KEY) is None


def test_truncated_ciphertext_degrades(on_windows):
    assert bc.decrypt_value(b"v10" + NONCE, KEY) is None
...
def test_the_mac_branch_does_not_try_gcm():
    ...
    assert bc.decrypt_value(p_win_blob(b"sessionid=abc123"), KEY) is None
